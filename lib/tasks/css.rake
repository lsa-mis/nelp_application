...
namespace :css do
  ...
  task :build do
    ...
    command = "sass app/scss:app/assets/builds --style=compressed --load-path=#{active_admin_path}"
  ...
  task :watch do
    active_admin_path = Gem::Specification.find_by_name("activeadmin").gem_dir + "/app/assets/stylesheets"
    command = "sass app/scss:app/assets/builds --style=compressed --load-path=#{active_admin_path} --watch"
